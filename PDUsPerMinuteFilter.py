    ...
    def Filter(self, pkt):
        ...
        if filterSource:
            if not Match(pkt.pduSource, filterSource):
                if _debug: PDUsPerMinuteTracer._debug("    - source filter fail")
                return
        if filterDestination:
            if not Match(pkt.pduDestination, filterDestination):
                if _debug: PDUsPerMinuteTracer._debug("    - destination filter fail")
                return
        if filterHost:
            if (not Match(pkt.pduSource, filterHost)) and (not Match(pkt.pduDestination, filterHost)):
                if _debug: PDUsPerMinuteTracer._debug("    - host filter fail")
                return
...
parser.add_argument(
    "-i", "--interval", nargs=1, type=int, default=60,
